...
- 在 `src/pages/index.html` 中搭建页面结构：
  ...
  - URL 监控列表区域。
  - 商品查询区域。
  - 顶部统一导出按钮及包含监控报警、异常 URL、商品导出的下拉菜单。
...
- tab 顺序固定为 `URL监控`、`商品查询`、`监控报警`。
- 页面首次访问默认展示 `URL监控` tab；刷新后恢复用户上次打开的 tab，并在监控报警 tab 数字中展示当前报警总数。
...
- 顶部仅展示一个 `导出` 按钮，点击后在按钮下方依次展示 `导出异常URL`、`导出商品`、`导出监控报警` 三项操作。
- 下拉菜单支持再次点击、点击外部、按 `Escape` 或选择操作后关闭。
- 保留现有监控报警和异常 URL 导出行为。
- 商品导出读取全部 `Product`，按更新时间倒序、商品名称排序，导出 `ID`、商品名称、规格、价格、库存、来源 URL、更新时间。
- 三类导出的 `ID` 统一按来源 URL 提取：jinritemai URL 使用 query string 中第一个小写 `id` 参数值，其他 URL 沿用 `.html` 前路径段的纯数字 ID。
- 商品导出中的价格和库存保持数字格式，不受当前筛选和分页影响；无商品时显示提示且不生成文件。

验收点：

- 首次打开页面默认展示 `URL监控`；已有页面状态时恢复上次打开的 tab。
- 监控报警列表能显示报警总数和报警明细。
- 监控报警列表按检查时间倒序展示。
...
- 实现 API 结果解析：
  ...
  - 跳过商品名称或规格为空的记录。
  - 将价格和库存转换为数字。
  - jinritemai 接口价格单位为分，解析后除以 `100` 并最多保留两位小数，再用于价格比较、报警生成和商品写入。
...
- 页面首次访问默认进入 `URL监控` tab；已有页面状态时恢复上次打开的 tab。
